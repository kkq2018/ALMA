...
def main(config):
    ...
    if not (config['evaluate'] or config['save_replay']) and config['use_wandb']:
        ...
        # save_path_base = os.path.join(wandb.run.dir, "*.th")
        # wandb.save(save_path_base)  # will push saved models to the cloud immediately, rather than waiting until the end of training
