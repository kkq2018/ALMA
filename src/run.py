...
def load_run(args, wb_run, learner, runner, logger, pi_only=False):
    ...
    files = wb_run.files()
    timesteps = set(int(f.name.split('/')[1].split('_')[0]) for f in files if f.name.endswith('.th'))
    ...
    files_to_load = [f for f in files if f.name.split('/')[-1].startswith(str(timestep_to_load))]
    if pi_only:
    ...
    model_path = os.path.join(model_dir, 'files', f"{timestep_to_load}_")
...
def run_sequential(args, logger):
    ...
    if args.checkpoint_run_name != "" or args.checkpoint_unique_id != "":
        ...
        if args.checkpoint_run_name != "":
            ...
            runs = [wandb_api.run(f'{args.wb_entity}/task_allocation/{args.checkpoint_unique_id}')]
